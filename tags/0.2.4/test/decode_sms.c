...
static unsigned short gsmToLatin1Table[] = {
    //  0 '@', '£', '$', '¥', 'è', 'é', 'ù', 'ì', 
...
{
    int i;
    LPSTR str = (LPSTR) malloc (len * 2 + 1);
    LPSTR basestr = str;
    if (*p++ & 0x50)
        *str++ = '+';
    for (i = 0; i < len; i++)
...
Gsm11Enc7bitToUTF8 (LPBYTE pData, int len)
{
    ...
    *str = 0;
    ...
    for (i = 0; i < str_len; i++)
    {
        wstr[i] = gsmToLatin1Table[base_str[i]];
    }
    wstr[i] = 0;
...
DecodeSMS (LPBYTE pRecData)
{
    ...
    if (smsc_len < 12)          //have meaning data.
    {
        ...
        if (*oa_a == 0xd0)      //for Alphanumeric address type
        {
            ...
            oa_phonenum = Gsm11Enc7bitToUTF8 (oa_a + 1, oa_len);
            oa_len += 1;
        }
        else
        {
